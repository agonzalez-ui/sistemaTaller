...
<article class="min-w-0 rounded-2xl border border-slate-200 p-5"><div class="flex flex-wrap items-center justify-between gap-2"><h2 class="break-words text-lg font-bold text-slate-900">{{ $brand->name }}</h2><span class="rounded-full px-3 py-1 text-xs font-semibold {{ $brand->active ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-100 text-slate-600' }}">{{ $brand->active ? 'Activa' : 'Inactiva' }}</span></div><p class="mt-3 text-sm text-slate-500">{{ $brand->vehicles_count }} vehículos registrados</p><div class="mt-4 flex flex-wrap gap-2"><a href="{{ route('vehicle-brands.edit',$brand) }}" class="inline-flex min-h-12 items-center rounded-xl bg-amber-50 px-4 py-3 font-semibold text-amber-800">{{ $brand->active ? 'Editar' : 'Editar / Reactivar' }}</a>
@if($brand->active)
<form action="{{ route('vehicle-brands.destroy',$brand) }}" method="POST" onsubmit="return confirm('¿Desea desactivar esta marca? Las motos existentes conservarán sus datos.')">@csrf @method('DELETE')<button type="submit" class="min-h-12 rounded-xl px-4 py-3 font-medium text-red-700 hover:bg-red-50">Desactivar</button></form>
@endif
</div></article>
@empty
